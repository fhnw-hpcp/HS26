...
#pragma omp barrier
    ...
    for (int output_rank = 0;
         output_rank < mpi_size;
         output_rank++) {
        if (output_rank == mpi_rank) {
            printf(
                "\n"
                "============================================================\n"
                "Host:                             %s\n"
            ...
            printf(
                ...
                (size_t)test_memory_per_rank_mb *
                    (size_t)mpi_size);

            for (int thread = 0;
                 thread < actual_thread_count;
                 thread++) {
                fputs(
                    thread_output +
                        (size_t)thread * OUTPUT_LINE_SIZE,
                    stdout);
